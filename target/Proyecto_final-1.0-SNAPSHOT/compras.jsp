...
<head>
    <meta charset="UTF-8">
    <title>Gestión de Compras - Sistema Empresarial</title>
    <style>
        ...
        * {
        ...
        /* ESTILOS CORREGIDOS PARA SELECTS - AHORA SE VEN BIEN */
        .form-select, .sel-prod {
        ...
        .form-select:focus, .sel-prod:focus {
            outline: none;
        ...
        /* LO MÁS IMPORTANTE: OPCIONES VISIBLES */
        .form-select option, .sel-prod option {
        ...
        .form-select option:hover, .sel-prod option:hover {
...
<body>
    ...
    <div class="top-nav">
        ...
        <div class="nav-menu">
            ...
            <a href="${pageContext.request.contextPath}/ADMIN/marcas.jsp" class="nav-link">Marcas</a>
            <a href="${pageContext.request.contextPath}/productos.jsp" class="nav-link">Productos</a>
        </div>
    </div>

    <div class="glass-container">
        <h1 class="section-title">Gestión de Compras</h1>
        
        <form action="sr_compra" method="post" id="frmCompra">
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">No. Orden</label>
                    <input type="number" class="form-input" name="txt_no_orden" placeholder="Número de orden" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" name="drop_proveedor" required>
                        <%
                            ...
                            DefaultTableModel tablaProveedores = pv.leer();
                            for (int i = 0; i < tablaProveedores.getRowCount(); i++) {
                                String id = tablaProveedores.getValueAt(i, 0).toString();
                                String nombre = tablaProveedores.getValueAt(i, 1).toString();
    ...
    <div class="glass-container history-section">
        <h3 class="history-title">Compras Recientes</h3>
        <div class="table-container">
    ...
    <script>
        // Catálogo vacío inicialmente - se cargará desde el servlet
        let catalogo = [];
        ...
        $(document).ready(function() {
            // Cargar productos y luego inicializar
            cargarProductos().done(function(data) {
                ...
                // Agregar primera fila después de cargar productos
        ...
        function filaDetalle(){
            ...
            var html = ''
              + '<tr>'
              +   '<td><select class="sel-prod">'+opts+'</select>'
              +       '<input type="hidden" name="detail_idProducto" class="idProd"></td>'
              +   '<td class="text-end exist"></td>'
              +   '<td class="text-end"><input type="number" step="0.01" min="0" class="table-input text-end costo" name="detail_costo" value=""></td>'
              +   '<td class="text-end"><input type="number" min="1" class="table-input text-end cant" name="detail_cantidad" value="1"></td>'
